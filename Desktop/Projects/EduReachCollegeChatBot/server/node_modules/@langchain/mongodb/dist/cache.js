...
//#region src/cache.ts
var MongoDBCache = class extends BaseCache {
	collection;
	PROMPT = "prompt";
	LLM = "llm";
	...
	async lookup(prompt, llmKey) {
		...
		try {
			...
			return arr.map((g) => deserializeStoredGeneration(JSON.parse(g)));
		} catch {
			return null;
		}
	}
	async update(prompt, llmKey, generations) {
		const serialized = JSON.stringify(generations.map((g) => JSON.stringify(serializeGeneration(g))));
...
var MongoDBAtlasSemanticCache = class MongoDBAtlasSemanticCache extends BaseCache {
	collection;
	embeddingModel;
	indexName;
	scoreThreshold;
...
//#endregion
export { MongoDBAtlasSemanticCache, MongoDBCache };
